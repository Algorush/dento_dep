<form action="{{$action}}" method="{{$method}}" id="{{$id}}" data-replace-blk="#{{$id}}" {!! $dynamic_fields !!}>
    ...
    <div class="card box-default">
        ...
        <div class="card-footer text-center">
            <div class="btn-group">
                @if($download_url)<a href="{{$download_url}}" type="button" class="btn btn-flat btn-info" download ><i class="fas fa-download"></i></a> @endif
                <button type="button" class="btn  btn-flat btn-success btn-file">
                    <i class="fas fa-edit"></i>
                    <input type="file" name="file" class="js_submit-form-by-change-el" accept="image/*" data-form="#{{$id}}">
                    {{--{{$name}}--}}
                </button>
