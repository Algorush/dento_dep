...
import {saveAsGlb} from '../../libs/saveAsGlb';
    ...
    init(){
        ...
        $el.filepond({
            ...
            oninitfile: function(file) {
                console.log(file)
                // convert OBJ to gltf with Draco (.drc file)
                const fileExt = file.fileExtension.toLowerCase();
                console.log(fileExt)
                if (fileExt == 'obj' || fileExt == 'ply') {
                    //file.abortLoad();
                    const objFileId = file.id;
                    const deleteFile = (fileId) => {
                        $el.filepond('removeFile', fileId);
                        console.log('file deleted')
                    }
                    saveAsGlb(file).then((drcFile)=>{

                        $el.filepond('addFile', drcFile)
                        .then(function (addedFile) {
                            console.log('file added');

                            deleteFile(objFileId);
                        });
