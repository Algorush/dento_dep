...
export default class Form {
  ...
  constructor (formElement, settings, reference) {
    ...
    this.onInit(listenerOnSubmit)
  }

   loadInputs() {
       return Array.from(this.form.querySelectorAll('input:not([type="hidden"]), select, textarea'));
   }
  ...
  onInit (listenerOnSubmit) {
    ...
    if (this.settings.onlyValidate) {
      ...
      $(this.form).on('submit', listenerOnSubmit);
  ...
  createInputsValidate () {
    ...
    this.inputs.forEach((el, i) => {
      let item = new Field(el, this.state, this.reference, this.settings, i, this.inputs)
  ...
  preSubmit () {
    ...
    this.error = false
    if (!this.isSpinnerActive) _this.addSpinner()
  ...
  prepareFiles (inputsWithFile, data) {
    ...
    const inputFiles = document.getElementsByClassName("upper_files_files_names");
