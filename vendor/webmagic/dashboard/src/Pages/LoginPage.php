...
class LoginPage extends ComplexElement
{
     ...
     */
    public function setDefaultForm(string $action = 'login', string $predefinedLogin = null, string $predefinedPassword = null)
    {
        $form = new FormGenerator();
        $form
            ->action(url($action))
            ->ajax(false)
            ->emailInput('email', old('email', $predefinedLogin), trans('dashboard::common.login_page.email'), true)
            ->passwordInput('password', $predefinedPassword, trans('dashboard::common.login_page.password'), true)
