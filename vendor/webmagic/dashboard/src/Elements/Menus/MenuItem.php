...
abstract class MenuItem extends RenderableElement implements AttributesAvailable
{
    ...
    public function checkOnAccess(): bool
    {
        ...
        //  if one of gates false - hide menu item
    ...
    /**
     * Check on access with Gates for submenu of laravel
     * @param MenuItem|null $sub_item
     * @return bool
     */
    public function checkOnAccessSubItem(MenuItem $sub_item): bool
    {
        if (empty($sub_item->gates)) {
            return true;
        }

        //  if one of gates false - hide sub item
        foreach ($sub_item->gates as $gate) {
            if (Gate::denies($gate)) {
                return false;
            }
        }
     ...
     * @return void
     */
    public function removeSubItem($sub_item_key)
    {
        $this->sub_items->forget($sub_item_key);
    }
