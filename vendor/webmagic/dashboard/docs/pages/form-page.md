...
$formPageGenerator = (new \Webmagic\Dashboard\Components\FormPageGenerator())
    ...
    // DateRange
    ->dateRange('dateRange', 'Date range', 'Y/m/d H:i', false, true, [['Current and previous months', Carbon::now()->subMonth()->startOfMonth(), Carbon::today()->endOfMonth()]])
    // Regular time picker
    ->timeInput('time', now(), 'Time')
    // Time picker JS
    ->timePickerJS('time_js', now(), 'Select date and time with JS')
